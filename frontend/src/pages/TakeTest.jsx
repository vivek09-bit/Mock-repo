...
const TakeTest = () => {
    ...
    const fetchTestAndUser = async () => {
      try {
        const [testRes, userRes] = await Promise.all([
          axios.get(`http://https://mock-repo-backend.onrender.com:5000/api/test/${testId}`),
          token
            ? axios.get("http://https://mock-repo-backend.onrender.com:5000/api/auth/me", {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post("http://https://mock-repo-backend.onrender.com:5000/api/test/submit", {
        testId,
